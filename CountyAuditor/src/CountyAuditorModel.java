...
public class CountyAuditorModel {
    ...
    public static String checkInput(String name, String zip, String state){
        ...
        if(!zip.matches("\\d+") || zip.length() != 5 || ZipCode.parseZip(zip, "CountyAuditor/Resources/zipcodes.csv") == null){
            return "Error: Invalid Zip Code";
        }
        if(!ZipCode.validateZip(zip, state, "CountyAuditor/Resources/zipcodes.csv")) {
    ...
    public synchronized static void exportCandidate(int id, Candidate candidate, String level){
        ...
        if(level == "Federal") {
            ...
            String[] candidateCounty = ZipCode.parseZip(candidate.getZip(), "CountyAuditor/Resources/zipcodes.csv");
